...
const EventsSection = () => {
  ...
  return (
    <section className="py-20 px-6 relative overflow-hidden" id="events">
      <KolamPattern className="absolute -right-20 top-10 w-[200px] h-[200px] opacity-40 animate-parallax-drift" />
      ...
      <div className="max-w-3xl mx-auto grid md:grid-cols-2 gap-6 md:gap-8 relative z-10">
        {events.map((event, i) => (
          <ScrollAnimate key={event.title} variant={event.variant} delay={i * 150}>
            <div className="glass rounded-2xl p-8 text-center h-full event-card-shimmer group magnetic-hover glass-lift">
              ...
              <div className="space-y-4 text-muted-foreground">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="font-sans text-sm font-semibold text-foreground animate-text-reveal">{event.date}</span>
                </div>
                <div className="flex items-center justify-center gap-2">
                  <Clock className="w-4 h-4 text-primary icon-hover-spin" style={{ transitionDelay: "100ms" }} />
                  <span className="font-sans text-sm">{event.time}</span>
                </div>
                <div className="flex items-center justify-center gap-2">
                  <MapPin className="w-4 h-4 text-primary icon-hover-spin" style={{ transitionDelay: "200ms" }} />
